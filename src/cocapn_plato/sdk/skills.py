...
class SkillScore:
    ...
    def update(self, usage: SkillUsage, decay_days: int = 7):
        ...
        self.avg_quality = (
            self.avg_quality * (self.total_uses - 1) + usage.outcome_quality
        ) / self.total_uses
...
class RateAwareSkill:
    ...
    def record_usage(
        self, skill_name: str, context: dict | None = None, outcome_quality: float = 0.5
    ) -> None:
    ...
    def get_relevant_skills(
        self, context: dict | None = None, top_k: int = 3
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_skill_stats(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            name: {
                ...
                "last_used": datetime.fromtimestamp(score.last_used).isoformat()
                if score.last_used
                else None,
            }
